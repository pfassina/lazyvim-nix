...
let
  ...
  deriveAutomaticParsers =
    extrasConfig: treesitterMappings: manualParsers:
    let
      # Get enabled extra names in "category.name" format
      enabledExtraNames = pkgs.lib.flatten (
        pkgs.lib.mapAttrsToList (
          category: extras:
          pkgs.lib.mapAttrsToList (
            name: extraConfig: pkgs.lib.optional (extraConfig.enable or false) "${category}.${name}"
          ) extras
        ) extrasConfig
      );
...
in
{
  test-parser-manifest-covers-runtime-languages =
    testLib.testNixExpr "parser-manifest-covers-runtime-languages"
      ''
        let
  ...
  test-latest-mode-missing-parser-fails-clearly =
  ...
  test-core-parsers-always-included = testLib.testNixExpr "core-parsers-always-included" ''
    let
      coreParsers = [
  ...
  test-enabled-extras-add-parsers = testLib.testNixExpr "enabled-extras-add-parsers" ''
    let
  ...
  test-disabled-extras-no-parsers = testLib.testNixExpr "disabled-extras-no-parsers" ''
    let
  ...
  test-manual-parsers-merged = testLib.testNixExpr "manual-parsers-merged" ''
    let
      coreParsers = ["lua" "vim"];
      extraParsers = ["rust"];
  ...
  test-parser-deduplication = testLib.testNixExpr "parser-deduplication" ''
    let
  ...
  test-extras-no-additional-parsers = testLib.testNixExpr "extras-no-additional-parsers" ''
    let
  ...
  test-multiple-extras-enabled = testLib.testNixExpr "multiple-extras-enabled" ''
    let
  ...
  test-parser-name-format = testLib.testNixExpr "parser-name-format" ''
    let
  ...
  test-enabled-extra-names = testLib.testNixExpr "enabled-extra-names" ''
    let
      extrasConfig = {
        lang = {
          rust = { enable = true; };
  ...
  test-parser-package-mapping = testLib.testNixExpr "parser-package-mapping" ''
    let
  ...
  test-empty-extras-config = testLib.testNixExpr "empty-extras-config" ''
    let
      emptyExtras = {};
      enabledNames = [];
  ...
  test-extract-lang-grammar-plugins = testLib.testNixExpr "extract-lang-grammar-plugins" ''
    let
  ...
  test-extract-lang-all-grammars = testLib.testNixExpr "extract-lang-all-grammars" ''
    let
      lib = (import <nixpkgs> {}).lib;
      # Updated extractLang supporting grammarName, language with associatedQuery, and detecting deprecated packages
      extractLang = pkg:
  ...
  test-extract-lang-grammarname-priority = testLib.testNixExpr "extract-lang-grammarname-priority" ''
    let
      lib = (import <nixpkgs> {}).lib;
      extractLang = pkg:
        let
  ...
  test-extract-lang-mixed = testLib.testNixExpr "extract-lang-mixed" ''
    let
      lib = (import <nixpkgs> {}).lib;
      extractLang = pkg:
        let
  ...
  test-extract-lang-deprecated-throws = testLib.testNixExpr "extract-lang-deprecated-throws" ''
    let
      lib = (import <nixpkgs> {}).lib;
      extractLang = pkg:
        let
  ...
  test-extract-lang-edge-cases = testLib.testNixExpr "extract-lang-edge-cases" ''
    let
      lib = (import <nixpkgs> {}).lib;
      extractLang = pkg:
        let
